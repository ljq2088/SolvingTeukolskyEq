...
def prefactor_Q(
    ...
    Q(r) = 1 + S(r),
    where
    S(r) = R_amp * r^(-p+2s) * (r-r_+)^p * exp(-2 i ω r_*(r))

    Returns
    -------
    Q     : prefactor_Q
    Q_r   : first derivative wrt r
    ...
    # gS = S'/S
    gS = alpha / r + p / drp - 2.0 * i * omega * rs_r

    # gS' = d/dr (S'/S)
    gS_r = -alpha / (r ** 2) - p / (drp ** 2) - 2.0 * i * omega * rs_rr
...
def Leaver_prefactors(
    ...
    term1 = drp**pp
    term2 = drm**pm
    exp_term = torch.exp(1j * omega * r)
    P = term1 * term2 * exp_term

    # log-derivative: g = P'/P
    g = pp / drp + pm / drm + 1j * omega

    # derivative of log-derivative
    gp = -pp / (drp**2) - pm / (drm**2)
