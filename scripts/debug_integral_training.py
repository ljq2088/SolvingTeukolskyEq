#!/usr/bin/env python3
"""Test: add integral consistency loss to single-point training.
If this fixes the PDE residual bias, the model should converge to the true solution.
"""
import sys
...
def benchmark_against_pybhpt(model, a, omega, u, v, lam, r_grid, M, m, s, device):
    dtype = torch.float64
    ...
    with torch.no_grad():
        ...
        slope = horizon_regularity_slope(a=a_t, omega=omega_t, lambda_=torch.tensor([lam], device=device, dtype=torch.complex128), m=m, M=M, s=s)
...
def main():
    ...
    configs = [
        ...
        ("PDE + Integral(0.1)", 0.1),
        ("PDE + Integral(1.0)", 1.0),
    ]

    for label, int_weight in configs:
        print(f"\n{'='*60}")
        print(f"  {label}")
        print(f"{'='*60}")
        ...
        for step in range(2000):
            ...
            pde_loss = loss.detach().item()

            if int_weight > 0:
                # Compute integral consistency loss
                f_int, f_y_int, _ = compute_f_derivatives_autograd(
                    model, a_t, omega_t, y_valid, u_batch=u_t, v_batch=v_t,
                )
                x_int = 0.5 * (y_valid + 1.0)
                A2_i, A1_i, A0_i = coeffs_x(
                    x=x_int.squeeze(0), a=a_t.squeeze(), omega=omega_t.squeeze(),
                    m=m_mode, lambda_=lam_t.squeeze(), s=s_val, M=M,
                )
                A2_int = A2_i.unsqueeze(0)
                A1_int = A1_i.unsqueeze(0)
                A0_int = A0_i.unsqueeze(0)
                ...
                loss_int = compute_integral_consistency_loss(
                    ...
                    a_batch=a_t, omega_batch=omega_t, lambda_batch=lam_t,
                    y_interior=y_valid,
                    _f=f_int, _f_y=f_y_int,
                    _A2=A2_int, _A1=A1_int, _A0=A0_int, _slope=slope_int,
                    n_anchors=16,
                )
                if torch.isfinite(loss_int):
                    loss = loss + int_weight * loss_int
            ...
            if step % 200 == 0:
                ...
                rel_err = benchmark_against_pybhpt(
                    model, a_c, omega_c, u_c, v_c, lam_c, r_grid, M, m_mode, s_val, device,
                )
                model.train()
                int_str = f" int={loss_int.item():.4e}" if int_weight > 0 else ""
                err_str = f" rel_err={rel_err:.4e}" if rel_err else ""
                print(f"  step {step:>5d}: PDE={pde_loss:.4f}{int_str}{err_str}")
