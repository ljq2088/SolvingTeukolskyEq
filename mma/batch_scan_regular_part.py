"""
批量扫描参数空间，计算并可视化 R' = R_in/U
参数范围:
  - a: 0 到 0.9
  - ω: 0.0001 到 0.3
  - l, m: 2 到 4
  - r: 2M 到 1000M (在 x=rp/r 上均匀采样)
...
OMEGA_VALUES = [0.0001, 0.01, 0.05, 0.1, 0.2, 0.3]
L_VALUES = [2, 3, 4]
M_VALUES = [2, 3, 4]
...
def plot_and_save(r, Rprime, s, l, m, a, omega, p, ramp, out_dir, show=True):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "Rprime_linear_abs.png"), dpi=150)
    if show:
        plt.show(block=False)
        plt.pause(0.5)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "Rprime_logr_re_im.png"), dpi=150)
    if show:
        plt.show(block=False)
        plt.pause(0.5)
    plt.close()
...
def main():
    ...
    try:
        ...
        # 生成所有参数组合
        param_list = list(product(A_VALUES, OMEGA_VALUES, L_VALUES, M_VALUES))
        ...
        for idx, (a, omega, l, m) in enumerate(param_list, 1):
            # 跳过 m > l 的情况
            if m > l:
                print(f"[{idx}/{total}] Skip: l={l}, m={m} (m > l)")
                continue
            ...
            # 绘图并保存（前几个显示，后面不显示以加速）
            show_plot = (idx <= 3)
            plot_and_save(r_mma, Rprime, S, l, m, a, omega, p, ramp, out_dir, show=show_plot)
