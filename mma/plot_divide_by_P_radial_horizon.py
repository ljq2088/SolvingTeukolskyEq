...
from physical_ansatz.prefactor import prefactor_P,r_star,delta,r_plus
...
A      = 0.1
OMEGA  = 0.1
RMIN   = 1.0+np.sqrt(1-A**2)+0.0001
RMAX   = 1.0+np.sqrt(1-A**2)+2.0
NPTS   = 500
M      = 1.0
...
_BASE_DIR = os.path.join(os.path.dirname(__file__), "divide_by_P_radial_horizon")
...
def compute_P(r_np):
    ...
    # P=r_t**3*torch.exp(1j*omega_t*r_star(r_t, a_t, M))
    P=delta(r_t, a_t, M)**(-S)*torch.exp(-1j*(omega_t-M_MODE*a_t/(2*r_plus(a_t, M)))*r_star(r_t, a_t, M))
    # P=r_t**3*torch.exp(1j*omega_t*r_star(r_t, a_t, M))
